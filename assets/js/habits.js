...
class HabitsModule {
    constructor() {
        ...
        this.categories = StorageManager.get(this.categoriesStorageKey, [
            ...
            { id: 'Productivity', label: 'Deep Work Productivity' },
            { id: 'Learning', label: 'Continuous Learning' }
        ]);

        this.habits = StorageManager.get(this.storageKey, [
            { id: 'habit_1', name: 'Read 30 mins technical docs', streak: 14, completedToday: true, category: 'Learning' },
            { id: 'habit_2', name: 'Core Workout & Stretching', streak: 5, completedToday: false, category: 'Wellness' },
            { id: 'habit_3', name: 'Deep Work Coding (2 hrs)', streak: 21, completedToday: true, category: 'Engineering' }
        ]);
        ...
        viewSection.innerHTML = `
            <div class="glass-card" style="display: flex; flex-direction: column; gap: 20px;">
                <div class="view-header-actions" style="margin-bottom: 0; flex-wrap: wrap; gap: 16px; justify-content: space-between; align-items: center;">
                    <div>
                        <h2>Habit Heatmaps & Consistency Matrix</h2>
                        <p class="date-subtitle">Monitor daily consistency and track execution streaks</p>
                    </div>
                    <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                        ...
                        <div style="position: relative; display: flex; align-items: center;">
                            ...
                            <input type="text" id="habitSearchInput" placeholder="Search habits..." value="${this.sanitizeHTML(this.searchQuery)}" style="padding: 6px 10px 6px 30px; border-radius: var(--radius-md); border: 1px solid var(--border-glass); background: var(--bg-main); color: var(--text-primary); font-size: 0.85rem; width: 150px;" />
                        </div>

                        <!-- Category Filter -->
                        <select id="habitCategoryFilter" class="form-control" style="padding: 6px 10px; border-radius: var(--radius-md); border: 1px solid var(--border-glass); background: var(--bg-main); color: var(--text-primary); font-size: 0.85rem;" title="Filter by Category">
                            <option value="all">All Categories</option>
                            ${this.categories.map(cat => `<option value="${cat.id}" ${this.selectedCategoryFilter === cat.id ? 'selected' : ''}>${cat.label}</option>`).join('')}
                        </select>

                        <!-- Manage Categories Button -->
                        <button class="btn btn-secondary" id="manageHabitCategoriesBtn" title="Manage Custom Categories" style="padding: 6px 10px; font-size: 0.85rem;">
                            <i class="fa-solid fa-tags"></i> Categories
                        </button>

                        <span class="badge soft-badge"><i class="fa-solid fa-chart-pie"></i> Today: ${completionRate}%</span>
                        <button class="btn btn-primary" id="addNewHabitBtn">
                            <i class="fa-solid fa-plus"></i> Add Habit
                        </button>
                    </div>
                </div>
                <div id="habitsListContainer" style="display: flex; flex-direction: column; gap: 16px;"></div>
            </div>
        `;
    ...
    renderHabitsListContent() {
        ...
        if (Array.isArray(filteredHabits) && filteredHabits.length > 0) {
            filteredHabits.forEach((habit) => {
                ...
                card.style.cssText = 'display: flex; justify-content: space-between; align-items: center; background: var(--bg-main); border-radius: var(--radius-md); padding: 18px; transition: transform var(--transition-fast), border-color var(--transition-fast);';
                
                card.innerHTML = `
                    <div style="display: flex; align-items: center; gap: 14px;">
                        <div style="background: ${habit.completedToday ? 'rgba(16, 185, 129, 0.1)' : 'var(--primary-light)'}; color: ${habit.completedToday ? 'var(--success)' : 'var(--primary)'}; width: 42px; height: 42px; border-radius: var(--radius-sm); display: flex; align-items: center; justify-content: center; font-size: 1.1rem; transition: background var(--transition-fast), color var(--transition-fast);">
                            <i class="fa-solid ${habit.completedToday ? 'fa-fire-flame-curved' : 'fa-fire'}"></i>
                        </div>
                        <div>
                            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                                <h4 style="font-size: 1rem; font-weight: 600; color: var(--text-primary); word-break: break-word;">${this.sanitizeHTML(habit.name)}</h4>
                            ...
                            <div style="display: flex; align-items: center; gap: 10px; margin-top: 2px;">
                                ...
                                <span style="font-size: 0.75rem; color: var(--text-secondary);">${habit.completedToday ? 'Finished for today' : 'Pending execution'}</span>
                            </div>
                        </div>
                    </div>
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <button class="btn ${habit.completedToday ? 'btn-primary' : 'btn-secondary'} btn-toggle-habit" data-index="${originalIndex}" style="padding: 8px 16px; font-size: 0.85rem;">
                            <i class="fa-solid ${habit.completedToday ? 'fa-check' : 'fa-circle'}"></i> ${habit.completedToday ? 'Completed' : 'Mark Done'}
                        </button>
                        <button class="btn btn-secondary btn-edit-habit" data-index="${originalIndex}" title="Edit Habit" style="padding: 8px 10px; font-size: 0.8rem;">
                            <i class="fa-solid fa-pen"></i>
                        </button>
                        <button class="icon-btn btn-delete-habit" data-index="${originalIndex}" title="Remove Habit" style="width: 36px; height: 36px; border-radius: var(--radius-sm); background: var(--bg-surface); border: 1px solid var(--border-glass); color: var(--danger); cursor: pointer; display: flex; align-items: center; justify-content: center; transition: var(--transition-fast);">
            ...
            container.innerHTML = `
                <div style="text-align: center; padding: 36px; color: var(--text-muted); font-size: 0.85rem; border: 1px dashed var(--border-glass); border-radius: var(--radius-md); background: var(--bg-main);">
                    <i class="fa-regular fa-fire" style="font-size: 1.8rem; margin-bottom: 8px; display: block; color: var(--text-muted);"></i>
                    No daily habits tracked or matching filters. Initialize a new habit matrix to begin consistency telemetry.
                </div>
            `;
        }
    }
            ...
            this.habits.slice(0, 3).forEach((habit) => {
                ...
                row.innerHTML = `
                    <div class="habit-info">
                        <i class="fa-solid ${habit.completedToday ? 'fa-fire-flame-curved' : 'fa-fire'}"></i>
                        <span>${this.sanitizeHTML(habit.name)}</span>
                    </div>
                    <button class="habit-check-btn ${habit.completedToday ? 'completed' : ''}" data-index="${originalIndex}" title="Toggle Habit">
            ...
            <div style="display: flex; flex-direction: column; gap: 14px;">
                ...
                <div style="border-top: 1px solid var(--border-glass); padding-top: 12px; display: flex; flex-direction: column; gap: 8px;">
                    ...
                    <div style="display: flex; gap: 8px;">
                        <input type="text" id="newHabitCategoryInput" placeholder="e.g., Fitness, Mindfulness..." autocomplete="off" style="flex: 1; padding: 10px; border-radius: var(--radius-md); border: 1px solid var(--border-glass); background: var(--bg-main); color: var(--text-primary); font-size: 0.85rem;">
                        <button class="btn btn-primary" id="saveNewHabitCategoryBtn" style="padding: 10px 16px;"><i class="fa-solid fa-plus"></i> Add</button>
                    </div>
                </div>
            </div>
        `;
    ...
    openHabitModal(editIndex = null) {
        ...
        const targetHabit = isEditing ? this.habits[editIndex] : { name: '', category: this.categories[0]?.id || 'Engineering', streak: 1, completedToday: false };
        ...
        const modalHTML = `
            <div class="form-group" style="display: flex; flex-direction: column; gap: 8px;">
                ...
                <input type="text" id="modalHabitName" value="${this.sanitizeHTML(targetHabit.name)}" placeholder="e.g., System Architecture Meditation & Review" autocomplete="off" style="padding: 12px; border-radius: var(--radius-md); border: 1px solid var(--border-glass); background: var(--bg-main); color: var(--text-primary);">
            </div>
            <div class="form-group" style="display: flex; flex-direction: column; gap: 8px; margin-top: 12px;">
                ...
                <select id="modalHabitCategory" style="padding: 12px; border-radius: var(--radius-md); border: 1px solid var(--border-glass); background: var(--bg-main); color: var(--text-primary);">
            ...
            ${isEditing ? `
                <div class="form-group" style="display: flex; flex-direction: column; gap: 8px; margin-top: 12px;">
                    ...
                    <input type="number" id="modalHabitStreak" min="0" value="${targetHabit.streak}" style="padding: 12px; border-radius: var(--radius-md); border: 1px solid var(--border-glass); background: var(--bg-main); color: var(--text-primary);">
                </div>
            ` : ''}
            <button class="btn btn-primary" id="modalSaveHabitBtn" style="margin-top: 16px; width: 100%;">
    ...
    deleteHabit(index) {
        if (this.habits[index]) {
            ...
            this.renderHabits();
            this.renderDashboardWidget();
            ComponentManager.showToast('Habit removed from tracking matrix.', 'info');
        }
    }
